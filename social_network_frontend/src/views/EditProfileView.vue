            ...
            <div class="p-12 bg-white border border-gray-200 rounded-lg">
                <Form class="space-y-6" :validation-schema="schema" @submit="submit_form" v-slot="{ errors }">
                    ...
                    <div class="mt-2 text-stone-700">
                        ...
                        <Field name="curr_password" class="mt-1" placeholder="Your current password"
                            :class="['w-full py-4 px-6 border rounded-lg', !errors.curr_password ? 'border-gray-200' : 'border-red-600']" />
                        <span name="curr_password" class="ml-2 text-red-600 text-xs line-clamp-6">
                            {{ errors.curr_password}}</span>
                    </div>
                    <div class="mt-2 text-stone-700">
                        <label>New Password</label><br />
                        <Field name="password" class="mt-1" placeholder="Your password"
                            :class="['w-full py-4 px-6 border rounded-lg', !errors.password ? 'border-gray-200' : 'border-red-600']" />
                        <span name="password" class="ml-2 text-red-600 text-xs line-clamp-6">
                            {{ errors.password}}</span>
                    </div>
                    <div class="mt-2 text-stone-700">
                        <label>Re-enter Password</label><br />
                        <Field name=confirm_password class="mt-1" placeholder="Re-enter new password"
                            :class="['w-full py-4 px-6 border rounded-lg', !errors.confirm_password ? 'border-gray-200' : 'border-red-600']" />
                        <span name=confirm_password class="ml-2 text-red-600 text-xs">{{ errors.confirm_password }}</span>
                    </div>
                    <div>
